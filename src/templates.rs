...
impl FillTemplate {
    ...
    pub fn fill(&self) -> String {
        ...
        let cycle_str = self.cycle.iter().map(|x| x.to_string()).collect::<Vec<String>>().join(", ");
        println!("{:?}", cycle_str);
        ...
        let group1values_tags = Self::generate_tags(String::from("groupvalues1"), self.group1values.len());
        ...
        let group2values_tags = Self::generate_tags(String::from("group2values"), self.group2values.len());
        let table_tags = Self::generate_tags(String::from("table"), self.find_number_of_tests());
        filled = self.value1.iter().enumerate().fold(filled, |acc, (i, item)| {value1_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &item.to_string()))});
        filled = self.test.iter().enumerate().fold(filled, |acc, (i, item)| {test_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &format!("{:?}", item)))});
        filled = self.upper1.iter().enumerate().fold(filled, |acc, (i, item)| {upper1_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &format!("{:?}", &item.to_string())))});
        filled = self.lower1.iter().enumerate().fold(filled, |acc, (i, item)| {lower1_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &format!("{:?}", &item.to_string())))});
        filled = self.value2.iter().enumerate().fold(filled, |acc, (i, item)| {value2_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &item.to_string()))});
		filled = self.upper2.iter().enumerate().fold(filled, |acc, (i, item)| {upper2_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &format!("{:?}", &item.to_string())))});
        filled = self.lower2.iter().enumerate().fold(filled, |acc, (i, item)| {lower2_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &format!("{:?}", &item.to_string())))});
        filled = self.group1values.iter().enumerate().fold(filled, |acc, (i, item)| {group1values_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &stringify_collection(item)))});
        filled = self.group2values.iter().enumerate().fold(filled, |acc, (i, item)| {group2values_tags.get(i).map_or(acc.clone(), |tag| acc.replace(tag, &stringify_collection(item)))});
        filled = table_tags.iter().enumerate().fold(filled, |acc, (i, item)| {table_tags.get(i).map_or(acc.clone(),|tag| acc.replace(tag, &format_vectors(&self.group1values[i], &self.group2values[i])))});
        filled = filled.replace(&String::from("\""), "");
        return filled;
    }

    pub fn generate_tags(tag : String, number_of_tags : usize ) -> Vec<String> {
        (0..number_of_tags).map(|i| format!("{{{{{}_{}}}}}", tag, i)).collect()
    }
...
mod tests {
    ...
    fn test_template_filling() {
        ...
        assert_eq!(filled_content, expected_filled_content, "The filled template does not match the expected content.");
    }
}
